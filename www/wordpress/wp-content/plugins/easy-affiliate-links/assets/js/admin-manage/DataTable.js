...
export default class DataTable extends Component {
    ...
    initDataTable( forceRefresh = false ) {
        ...
        if ( false !== selectedColumns ) {
            let savedSelectedColumns = localStorage.getItem( `eafl-admin-manage-${ this.props.options.id }-columns` );
    ...
    onColumnsChange(id, checked) {
        ...
        localStorage.setItem( `eafl-admin-manage-${ this.props.options.id }-columns`, JSON.stringify( selectedColumns ) );
    }

    requirementMet() {
        if ( this.props.options.hasOwnProperty( 'required' ) && ( ! eafl_admin.addons.hasOwnProperty( this.props.options.required ) || true !== eafl_admin.addons[ this.props.options.required ] ) ) {
            return false;
        }
    ...
    render() {
        ...
        if ( ! this.requirementMet() ) {
            ...
            return (
                <div className="eafl-admin-manage-requirement">
                    <div>*{ __eafl( 'This feature is only available in' ) }</div>
                    <a href="https://bootstrapped.ventures/easy-affiliate-links/get-the-plugin/" target="_blank">
                        { `Easy Affiliate Links ${bundle} Bundle` }
                    </a>
                </div>
            );
        }
        ...
        if ( eafl_admin_manage_modal.settings.hasOwnProperty( 'default_page_size' ) ) {
            let settingsPageSize = parseInt( eafl_admin_manage_modal.settings.default_page_size );

            if ( ! isNaN( settingsPageSize ) && 0 < settingsPageSize ) {
                defaultPageSize = settingsPageSize;
            }
        }
            ...
            <div className="eafl-admin-manage-page">
                {
                    ...
                    <div className="eafl-admin-manage-header">
                        ...
                        <div className="eafl-admin-manage-header-buttons">
                            {
                                ...
                                >{ __eafl( 'Bulk Edit' ) } { this.getSelectedRows().length } { 1 === this.getSelectedRows().length ? this.props.options.label.singular : this.props.options.label.plural }...</button>
                            }
                            {
                                this.props.options.createButton
                                ?
                ...
                <div className="eafl-admin-manage-table-container">
                    <ErrorBoundary module="Datatable">
                        ...
                        />
                        <div className="eafl-admin-manage-table-inner">
                            <ReactTable
                                ...
                                defaultPageSize={ this.props.options.hasOwnProperty( 'defaultPageSize' ) ? this.props.options.defaultPageSize : defaultPageSize }
